...
public class DateTimeTutor1 {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "Your birthday is " + (nextBirthday.isAfter(LocalDate.now()) ? "ahead" : "behind"));
    ...
    int offsetNow = TimeZone.getTimeZone("America/New_York")
        .getOffset(Instant.now().toEpochMilli());
    System.out.println(TimeUnit.MILLISECONDS.toHours(offsetNow));
    int rawOffset = TimeZone.getTimeZone("America/New_York").getRawOffset() - TimeZone.getDefault()
        .getRawOffset();
    ...
    LocalDateTime parsedTime = LocalDateTime
        .parse(time, DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm"));
